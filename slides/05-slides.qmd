...
What's the relationship between drinking coffee and lung cancer?
...
coffee_dag1 %>% tidy_dagitty(layout = "linear") %>% 
  ...
  geom_dag_label(aes(label =label),nudge_y =.25) +
  ylim(-1,1)+
  theme_void()
...
coffee_dag<- coffee_dag %>% 
  tidy_dagitty() 

ggdag(coffee_dag,
      text = F,
      use_labels = "label")+
  theme_void() -> confounder_fig1
