...
covid_us %>%
  ...
  mutate(
    ...
    new_deaths_pc_14da = zoo::rollmean(new_death_pc, k = 14, align = "right", fill=NA ),
...
df_feedback_full %>%
  ...
  ungroup()%>%
  na.omit()%>%
  ggplot(aes(Options,Vote))+
  geom_bar(stat = "identity")+
  facet_wrap(~Fit, scale="free_y",ncol = 1)+
  coord_flip()
...
- A Regression Discontinuity Design (RDD) leverages a naturally occuring discontinuity in the probability of receiving some treatment, and uses regression to estimate the effect of that treatment at the discontinuity

- The identifying assumption of a RDD is either:
...
- Common discontinuties in political science:
...
.left-column[
- No path from $Z$ to $Y$ except through $D$
]
