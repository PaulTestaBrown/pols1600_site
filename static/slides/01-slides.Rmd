...
Today we'll explore COVID-19 data in the U.S.

- We'll try to answer to one question. On average, were new cases lower in states that had stay at home orders
...
3. Install the `tidyverse` and `COVID19` packages
...
---
## Excerise 2: Create some simple objects
...
--- 
## Excerise 2: Create some more complex objects
...
---
class: inverse, center, middle
background-image: url("https://img.resized./data-scientist-meme-main.jpg")
background-size:cover

# 💪 
# 3. Installing packages and reading data
...
# Excerise 5. Data Wrangling with the Tidyverse
...
length(unique(covid_us$administrative_area_level_2))

covid_us %>%
  mutate(
    state = administrative_area_level_2,
    year-m= lubridate::ym(lubridate::ymd(date))
  ) %>%
  ...
  mutate(
    ...
    face_masks = case_when(
      #facial_coverings < 0 ~ NA_character_,
...
library(lubridate)
year(lubridate::ymd(covid_us$date))
year(covid_us$date)
month(covid_us$date)
table(abs(covid_us$facial_coverings),useNA = "ifany")
table(covid_us$face_masks,useNA="ifany")
table(covid_us$new_cases < 0)
covid_us%>%
  group_by(face_masks) %>%
  summarise(
...
covid_us %>%
  ...
  ggplot(aes(date,n, col=face_masks))+
