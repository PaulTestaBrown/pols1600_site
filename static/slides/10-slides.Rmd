...
df_feedback_full <- haven::read_spss("../files/data/wk08.sav")
df_feedback_full %>%
...
df_feedback %>%
  mutate(
    Meteor = stringr::str_wrap(forcats:: as_factor(apocalypse),20)
  ) %>%
  ggplot(aes(Meteor, fill = Meteor))+
  geom_bar(stat="count")+
  guides(fill = "none")+
  coord_flip()+
  labs(x="",
       title = "There's a giant meteor headed toward Earth...")
...
df_feedback %>%
  ...
  mutate(
    treatment = ifelse(version == "list_a", "Direct","Veiled")
  )%>%
  group_by(treatment)%>%
  summarize(
...
df_feedback %>%
  ...
  na.omit()%>%
  ggplot(aes(response, N, fill = treatment))+
  geom_bar(stat="identity")+
