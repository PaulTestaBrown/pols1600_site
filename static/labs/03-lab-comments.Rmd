...
groups_df <- tibble::tibble(
  `Group 1` = c("Maria Sofia Pena", "Lily Secondorf", 
...
- In the `mutate()` function, change:
  ...
  - the values `No Survey` to `Recyclying` and `Survey` to `Trans-Equality`
...
df %>%
  ...
  summarise(across(starts_with("vf_"), mean))%>%
  # Gather columns containing means, exclude column for groups
  gather(key = "Covariate", 
         value = "Mean", 
         -treatment_assigned) %>%
  # Put No Survey and Survey means in separate columns
  spread(key = treatment_assigned, value = Mean)%>%
  # Calculate difference
  mutate(
...
- In the `mutate()` function, change:
  ...
  - Add the following line `Days = c(0,3,21,42,90)` to create a column called Days (*make sure to put a comma after `ATE = `Trans-Equality` - Recycling,` )
...
df %>%
  ...
  gather(key = "Outcome", 
         ...
         -treatment_group) %>%
  # Put No Survey and Survey means in separate columns
  spread(key = treatment_group, value = Mean)%>%
  # Calculate difference
  mutate(
