...
1. Summarize the study in terms of it's research question, theory, design, and results.

2. Download the replication files and get set up to work (15 minutes)
...
Please see the [slides](https://pols1600.paultesta.org/slides/07-slides#/go-to-the-papers-dataverse)

Make sure the files are saved in to the SAME folder as this lab
...
In the code chunk below, please take a quick look at each dataset and answer the following for each dataset:
...
To merge `data` into `cps`, we first need to merge the `st` column (State postal abbreviations of) from `fips_codes` into `cps` by the common `statefip` variable that is common to both `cps` and `state_fip`

Then we can merge `policy_data` into `cps` by the common variables `st` and `year` to match the policies in given state and year to observations in the cps from that state in that year.

To avoid complications that from running merge code multiple times, we will save the output each merge into temporary data files, and then we're confident we have done this correctly, will save the final merged data back into CPS.
...
Now we can merge the policy data from `data` into `tmp1` using the common columns of `st` and `year` in each data frame.
...
And quickly check that output of our merge worked correctly
...
Finally, following Grumbach and Hill, we will remove DC and ND from the data and filter the data to start at 1978
...
Before saving the merged data back into `cps`
...
The **treatment** is the effect of living in  **same day registration** which is captured by the `sdr` variable.

The **key predictor** will be indicators of a respondent's **age_cohort** which we **need to create** from the `age` variable. 

Next lab we will also look at models that the following covariates:  
...
Next we'll create an `age_group` variable, and binary indicators for each age cohort.

Please uncoment and run the code below
...
- **Look** at the variable `voted` using the `table()` function
  ...
  - `96`,`97`,`98` to people who didn't provide and answer, or didn't rember
  - `99` corresponds to people [who shouldn't be in the sample ("Not in universe")](https://cps.ipums.org/cps-action/faq)
...
## Recreate Figure 1 describe the treatment
...
## Calculate the proption of voting by age group and SDR

With the `cps` data, use `group_by()` and summarize to calculate the propotion of people voting by age group in states that did and did not have same day registration in the code chunk below.
...
**What does Figure 2 tell us?** Figure 2 provides initial support of Grumbach and Hill's general argument. The propotion of people voting in states with SDR is higher than states with out SDR, and this gap is larger among younger age cohorts.
